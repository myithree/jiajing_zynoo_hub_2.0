	...
	switch (commade_case) // 命令字
	{
		...
		case GIF_TRANS_PACKAGE_TRANS_COMMAND:												//----------------------------------
		  ...
		  // 更新下一个包存储的起始地址,第一包包含传输信息共长Nondata_Part_Len，减去对应长度信息
			flash_qspi_writebuffer((uint8_t *)receive_data, Updata_state_adderss, Vcp_Rx_Data_length - Nondata_Part_Len);	

			Updata_state_adderss = Updata_state_adderss + Vcp_Rx_Data_length - Nondata_Part_Len;	//更新存储数据索引		

			crc_data_reset();
			crc_block_calculate((uint32_t*)receive_data, (Vcp_Rx_Data_length - Nondata_Part_Len)/4);
		...
		spend_time=getCurrentMicros()-start_time;		
