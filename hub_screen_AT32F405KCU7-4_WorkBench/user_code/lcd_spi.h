...
#define LCD_Height    320	//- 220	// LCD的像素宽度
...
void LCD_WriteData_16bit(uint16_t x);
...
#define 	LCD_CS_H    		 	gpio_bits_set(LCD_CS_PORT, LCD_CS_PIN)							// 片选拉高
#define 	LCD_CS_L     			gpio_bits_reset(LCD_CS_PORT, LCD_CS_PIN)        	// 片选拉低

#define  	LCD_DC_Command		 gpio_bits_reset(LCD_DC_PORT, LCD_DC_PIN)	  		// 低电平，指令传输 
#define 	LCD_DC_Data		     gpio_bits_set(LCD_DC_PORT, LCD_DC_PIN)				// 高电平，数据传输

#define  	RST_1		 					gpio_bits_set(GPIOA, GPIO_PINS_15)	  		// 低电平，指令传输 
#define 	RST_0		     			gpio_bits_reset(GPIOA, GPIO_PINS_15)				// 高电平，数据传输

#define 	LCD_Backlight_ON     gpio_bits_set(LCD_Backlight_PORT, LCD_Backlight_PIN)     // 高电平，开启背光
#define 	LCD_Backlight_OFF  	 gpio_bits_reset(LCD_Backlight_PORT, LCD_Backlight_PIN)			  // 低电平，关闭背光
